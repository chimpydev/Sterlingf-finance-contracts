...
contract LPRewardsTest is BaseTest {
    ...
    function setUp() public {
        ...
        amounts[0] = 2 * TOKEN_1M; // use 1/2 for veSTERLING position
        amounts[1] = TOKEN_1M;
        mintSterling(owners, amounts);

        // give owner1 veSTERLING
        VeArtProxy artProxy = new VeArtProxy();
        escrow = new VotingEscrow(address(STERLING), address(artProxy));
        STERLING.approve(address(escrow), TOKEN_1M);
    ...
    function testLPsEarnEqualSterlingBasedOnVeSterling() public {
        // owner1 deposits LP
