...
contract PairTest is BaseTest {
    ...
    function votingEscrowMerge() public {
        ...
        assertEq(STERLING.balanceOf(address(escrow)), 2 * TOKEN_1);
        ...
        assertEq(STERLING.balanceOf(address(escrow)), 3 * TOKEN_1);
        console2.log(escrow.totalSupply());
        escrow.merge(3, 1);
        console2.log(escrow.totalSupply());
        assertGt(escrow.balanceOfNFT(1), 1990063075414519385);
    ...
    function deployPairFactoryGauge() public {
        ...
        STERLING.approve(address(gaugeFactory), 15 * TOKEN_100K);
        ...
        staking = new TestStakingRewards(address(pair), address(STERLING));
    ...
    function addGaugeAndBribeRewards() public {
        ...
        STERLING.approve(address(gauge), PAIR_1);
        STERLING.approve(address(xbribe), PAIR_1);
        STERLING.approve(address(staking), PAIR_1);

        gauge.notifyRewardAmount(address(STERLING), PAIR_1);
        xbribe.notifyRewardAmount(address(STERLING), PAIR_1);
        staking.notifyRewardAmount(PAIR_1);

        assertEq(gauge.rewardRate(address(STERLING)), 1653);
        // no reward rate, all or nothing
        // assertEq(xbribe.rewardRate(address(STERLING)), 1653);
    ...
    function createLock2() public {
        ...
        assertGt(escrow.balanceOfNFT(1), 995063075414519385);
        assertEq(STERLING.balanceOf(address(escrow)), 4 * TOKEN_1);
    }
    ...
    function gaugeDistributeBasedOnVoting() public {
        ...
        STERLING.approve(address(voter), PAIR_1);
    ...
    function minterMint() public {
        ...
        console2.log(STERLING.balanceOf(address(distributor)));
        console2.log(distributor.claimable(1));
        uint256 claimable = voter.claimable(address(gauge));
        STERLING.approve(address(staking), claimable);
    ...
    function gaugeClaimRewards() public {
        ...
        gauge.batchRewardPerToken(address(STERLING), 200);
        vm.warp(block.timestamp + 1);
        uint256 earned = gauge.earned(address(STERLING), address(owner));
        address[] memory rewards = new address[](1);
        rewards[0] = address(STERLING);
        vm.warp(block.timestamp + 1);
    ...
    function minterMint2() public {
        ...
        token[0] = address(STERLING);
        tokens[0] = token;
        voter.claimRewards(gauges, tokens);
        assertEq(gauge.rewardRate(address(STERLING)), staking.rewardRate());
        console2.log(gauge.rewardPerTokenStored(address(STERLING)));
    }
    ...
    function gaugeClaimRewardsOwner3NextCycle() public {
        ...
        owner3.approve(address(pair), address(gauge), PAIR_1);
        owner3.deposit(address(gauge), PAIR_1, 0);
        uint256 before = STERLING.balanceOf(address(owner3));
        vm.warp(block.timestamp + 1);
        // uint256 earned = gauge.earned(address(STERLING), address(owner3));
        address[] memory rewards = new address[](1);
        rewards[0] = address(STERLING);
        owner3.getGaugeReward(address(gauge), address(owner3), rewards);
        uint256 after_ = STERLING.balanceOf(address(owner3));
        uint256 received = after_ - before;
        assertGt(received, 0);
        console2.log(gauge.rewardPerTokenStored(address(STERLING)));
    ...
    function testGaugeClaimRewards3() public {
        ...
        pair.approve(address(gauge), PAIR_1);
        gauge.deposit(PAIR_1, 0);
        STERLING.approve(address(gauge), STERLING.balanceOf(address(owner)));
        gauge.notifyRewardAmount(address(STERLING), STERLING.balanceOf(address(owner)));
